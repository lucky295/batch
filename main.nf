...
process remove_contaminants {
    ...
    if (params.singleEnd){
       ...
       """
	echo 'CMD: bbmap.sh in=${name}.fastq.gz outu=${name}.clean.fastq.gz ref=${my_contaminant_fasta} maxindel=1 minid=0.95 nodisk statsfile=${name}.trimstats.txt'	  
       ...
       """
        echo 'CMD: bbmap.sh in1=${name}_1.fastq.gz in2=${name}_2.fastq.gz outu1=${name}_1.clean.fastq.gz outu2=${name}_2.clean.fastq.gz ref=${my_contaminant_fasta} maxindel=1 minid=0.95 nodisk statsfile=${name}.trimstats.txt'
...
process remove_adapters {
    ...
    if (params.singleEnd){
       ...
       """
       echo 'CMD: fastp --in1=${name}.clean.fastq.gz  --out1=${name}_trimmed_clean.fastq.gz  --report_title=${name}_FASTP  --json=${name}_fastp.json  --html=${name}_fastp.html'     
      ...
      fastp --in1=${name}.clean.fastq.gz \\
       ...
       """
       echo 'CMD: fastp --in1=${name}_1.clean.fastq.gz --in2=${name}_2.clean.fastq.gz --out1=${name}_1_trimmed_clean.fastq.gz --out2=${name}_2_trimmed_clean.fastq.gz  --report_title=${name}_FASTP  --json=${name}_fastp.json  --html=${name}_fastp.html'
...
process quant {
    ...
    if (params.singleEnd){
       ...
       """
	echo 'CMD: salmon quant  --seqBias --validateMappings --numBootstraps 100 -l A -i ${params.salmon_index_path} -r ${reads} -o salmon_${pair_id} '
	...
	salmon quant --threads 16 --seqBias --validateMappings --numBootstraps 100 -l A \\
     ...
     if(my_quant_method == 'salmon') {
       ...
       """
       echo 'CMD: salmon quant --seqBias --validateMappings --numBootstraps 100 -l A -i ${params.salmon_index_path} -1 ${pair_id}_1.trimmed.clean.fastq.gz -2 ${pair_id}_2.trimmed.clean.fastq.gz -o salmon_${pair_id}'
...
process multiQC {
    ...
    input:
    file multiqc_config
    file (fastqc:'pre_fastqc/*') from  preqc_results.collect()
    file ('qc/bbmap/stats/*') from contaminant_stats.collect()
    file ('qc/adapter_removal/*') from adapter_removal_stats.collect()
    file ('qc/fastqc/*') from fastqc_after_results.collect()
    file ('quant/*') from quant_ch.collect()
    ...
    """
    echo 'CMD: multiqc . -f ${rtitle} ${rfilename} --config ${multiqc_config}'
